        ...
        fn update(&mut self, cqe_res: i32, code: u8) -> Option<(types::Fd, usize)> {
            match code {
                ...
                opcode::Statx::CODE => {
                    ...
                    self.read_size = Some(
                        ((s / _ALIGNMENT + u64::from(s % _ALIGNMENT != 0)) * _ALIGNMENT) as usize,
                    );
            ...
            while n_received_results < n_files {
                if idx < n_files && n_in_flight < self.ring.completion().capacity() - 1 {
                    let space_left = unsafe {
                        self.ring.submission_shared().capacity()
                            - self.ring.submission_shared().len()
                    };
                    ...
                    if space_left > 2 {
                        ...
                        let l = unsafe {
                            self.ring.submission_shared().len()
                                + self.ring.completion_shared().len()
                        };
                ...
                for cqe in unsafe { self.ring.completion_shared() } {
                    ...
                    match ud.op {
                        opcode::OpenAt::CODE | opcode::Statx::CODE => {
                            if let Some((fd, s)) =
                                files_in_flight[op_idx].update(cqe.result(), ud.op)
                            {
...
mod fallback_impl {
    ...
    impl WorkerPool {
        pub fn new(_ring_size: u32, _n_workers: usize) -> ZarrResult<Self> {
            Err(ZarrError::Read(
                "io-uring is not supported on this platform".to_string(),
            ))
        }
        ...
        pub fn add_file(&mut self, _filename: String) -> ZarrResult<()> {
            Err(ZarrError::Read(
                "io-uring is not supported on this platform".to_string(),
            ))
        }
